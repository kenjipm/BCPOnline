...
class Bidding_live_model extends CI_Model {
	
	private $table_bidding_live = 'bidding_live';
	...
	// get bidding_live detail
	public function get_from_id($id)
	{
		$where['id'] = $id;
		
		$this->db->where($where);
		$query = $this->db->get($this->table_bidding_live, 1);
		$bidding_live = $query->row();
		
		return ($bidding_live !== null) ? $this->get_stub_from_db($bidding_live) : null;
	}
	...
	{
		$query = $this->db->get($this->table_bidding_live);
		$items = $query->result();
	...
	public function get_all_from_posted_item_id($posted_item_id)
	{
		...
		$this->db->select('*, ' . $this->table_bidding_live.'.id AS id, customer.id AS customer_id, account.id AS account_id');
		$this->db->join('customer', 'customer.id=' . $this->table_bidding_live . '.customer_id', 'left');
	...
	public function insert_from_stub()
	{
		...
		$this->db->insert($this->table_bidding_live, $db_item);
	...
	public function generate_winner_message($posted_item_name, $price, $billing_id)
	{
		...
		return "Selamat! Anda memenangkan lelang untuk barang " . $posted_item_name .
