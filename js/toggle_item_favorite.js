function toggle_item_favorite(posted_item_id) {
	$.ajax({
		...
		success: function(data) {
			if (data == "1") {
				$("#btn-toggle_item_favorite").removeClass('cb-button-form');
				$("#btn-toggle_item_favorite").addClass('cb-button-secondary-selected');
				$("#btn-toggle_item_favorite").html('Sudah Difavoritkan');
			}
			else if (data == "0") {
				$("#btn-toggle_item_favorite").removeClass('cb-button-secondary-selected');
				$("#btn-toggle_item_favorite").addClass('cb-button-form');
